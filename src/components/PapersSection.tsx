...
export function PapersSection() {
  ...
  const [developerPapers, setDeveloperPapers] = useState<Paper[]>([]);
  const [combinedPapers, setCombinedPapers] = useState<Paper[]>([]);
  ...
  useEffect(() => {
    ...
    const devPapers = getDeveloperPapers();
    setDeveloperPapers(devPapers);

    // Combined: top 6 + all papers, sorted together
    const combined = [...rawTop, ...rawAll].sort((a, b) => {
      if (sortMode === 'year') return b.year - a.year;
      return (b.citationCount || 0) - (a.citationCount || 0);
    });
    setCombinedPapers(combined);
  }, [sortMode]);
  ...
  const displayPapers = expanded ? combinedPapers : topPapers;
      ...
      <div className={`max-w-6xl mx-auto transition-all duration-1000 delay-200 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <div className="mt-8 text-center">
          ...
          >
            ...
            <span>{expanded ? 'View less' : 'View all publications'}</span>
          </button>
        </div>

        {/* Developer Papers Section */}
        <div className="relative my-12">
          <div className="absolute inset-0 flex items-center">
            <div className="w-full border-t border-white/20"></div>
          </div>
          <div className="relative flex justify-center">
            <span className="px-4 py-2 bg-navy-800 text-white/50 text-sm rounded-lg flex items-center gap-2" style={{ fontFamily: 'var(--font-body)' }}>
              <span className="text-xl">😊</span>
              <span>This is <strong className="text-mint-400">our own paper</strong> — we're excited to share it with you! Feel free to read.</span>
            </span>
          </div>
        </div>
        ...
        {/* Developer Papers */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {developerPapers.map((paper, index) => (
            <article key={paper.doi || paper.title} className="group relative bg-mint-400/10 border border-mint-400/30 rounded-xl p-6 hover:border-mint-400/50 transition-all duration-300">
              <div className="flex items-start justify-between mb-4">
                <span className="text-mint-400 font-bold text-lg" style={{ fontFamily: 'var(--font-mono)' }}>D{index + 1}</span>
                <div className="flex items-center gap-2">
                  <span className="text-sm text-white/50" style={{ fontFamily: 'var(--font-body)' }}>{paper.year}</span>
                  {paper.citationCount !== null && (
                    <span className="inline-flex items-center px-2 py-1 rounded text-xs bg-mint-400/20 text-mint-400" style={{ fontFamily: 'var(--font-mono)' }}>
